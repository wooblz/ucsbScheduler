...
func TestInsertClass(t *testing.T)  {
    ...
    queryClass,err := db.Prepare(`SELECT id, course_id, title, subject_area, enroll_code, room, building, days, begin_time, end_time FROM classes WHERE course_id = $1`)
    if err != nil  {
    ...
    var title, courseID, subjectArea, room, enrollCode, building, days, begin_time, end_time string
    var class_id int
    for _, class := range Test1  {
        mistake := false
        err = queryClass.QueryRow(class.CourseID).Scan(&class_id, &courseID, &title, &subjectArea, &enrollCode, &room, &building, &days, &begin_time, &end_time)
        if err != nil  {
        ...
        err = querySection.QueryRow(class_id).Scan(&section_id, &enrollCode)
        if err != nil  {
            t.Fatalf("Failed to query row: %v", err)
        }
        row, err := queryTime.Query(section_id)
        if err != nil  { 
            t.Fatalf("Failed to query row: %v",err)
        }
        defer row.Close()
